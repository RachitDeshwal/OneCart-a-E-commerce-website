...
const Nav = () => {
    ...
    const handleLogOut=async()=>{
        try{
            const result=await axios.get(`${serverUrl}/api/auth/logout`,{},{withCrediantials:true})
            console.log(result)
            navigate("/login")
  ...
  return (
    <div className='w-[100vw] h-[70px] fixed top-0 flex items-center justify-between bg-[#dcdbdbf8] z-10 px-[30px] overflow-x-hidden shadow-md shadow-black '>
        ...
        <button className='text-[15px] rounded-2xl text-white px-[20px] py-[10px] bg-[#000000ca] border-[#89daea] hover:border-[2px] cursor-pointer' onClick={()=> handleLogOut()}>LogOut</button>
    </div>
  )
}
